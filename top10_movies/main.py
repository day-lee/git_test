...
@app.route("/")
def home():
    """Return a list of objects from DB and passes data to template"""
...
def add():
    ...
    if form.validate_on_submit():
        ...
        response              = requests.get(MOVIE_API_ENDPOINT, params=parameters)
...
def find():
    ...
    if movie_api_id:
        ...
        detail_response      = requests.get(f"{MOVIE_API_ENDPOINT}{movie_api_id}", params=parameters)
